...
export const MainBanner = () => {
  ...
  return (
    <section className="container mx-auto  relative group overflow-hidden bg-gray-100 rounded-3xl" ref={emblaRef}>
      <div className="flex">
        {/* Слайд 1 */}
        <div className="flex-[0_0_100%] min-w-0 relative h-100 md:h-125">
      ...
      <button
        className="absolute hover:bg-(--color-green) hover:text-white left-4 top-1/2 -translate-y-1/2 bg-white/80 p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
        onClick={scrollPrev}
      >
        ←
      </button>
      <button
        className="absolute hover:bg-(--color-green) hover:text-white right-4 top-1/2 -translate-y-1/2 bg-white/80 p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
        onClick={scrollNext}
      >
        →
      </button>
